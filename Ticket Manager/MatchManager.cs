    ...
    internal class MatchManager
    {
        ...
        public List<Auction> GetAuctionsSearchPrompt(string search)
        {
            var allAuctions = this.matches
                    ...
                    || a.Key.Match.TeamHome.ToLower().Contains(search.ToLower())
                    || a.Key.Match.TeamAway.ToLower().Contains(search.ToLower())
                    || a.Key.Stadium.Town.ToLower().Contains(search.ToLower())
                    || a.Key.Stadium.Name.ToLower().Contains(search.ToLower()))
                    .Select(a => a.Key)
                    .ToList();
